    ...
    if (seed.render === "enum" && (seed.options || {}).enum) {
      const options =
        typeof seed.options.enum === "object"
          ? Object.entries(seed.options.enum).map(([value, label]) => ({
              value,
              label
            }))
          : seed.options.enum;
      ...
      column.dataValidation = {
        type: "list",
        allowBlank: true,
        formulae: [`"${options.map(({ value }) => value).join(",")}"`]
      };
      // 注解
      column.note = {
        texts: [
  ...
  columns.forEach((column, columnIndex) => {
    ...
    if (column.dataValidation) {
      ...
      worksheet.dataValidations.add(
        `${columnName}2:${columnName}9999`,
        column.dataValidation
      );
    }
    // 备注
    if (column.note) {
      worksheet.getCell(`${columnName}1`).note = column.note;
    }
  });
